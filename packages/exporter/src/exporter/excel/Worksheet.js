 ...
 *
 */
Ext.define('Ext.exporter.excel.Worksheet', {
    extend: 'Ext.exporter.excel.Workbase',
    ...
    worksheetTpl: new Ext.XTemplate(
        '<ss:Worksheet ss:Name="{sheetName}">',
            '<ss:Names>',
                '<ss:NamedRange ss:Name="Print_Titles" ss:RefersTo="=\'{sheetName}\'!R1:R2" />',
            '</ss:Names>',
    ...
    render: function () {
        ...
        return me.worksheetTpl.apply({
            ...
            sheetName:  Ext.String.ellipsis(me.title, 30),
    ...
    buildRows: function () {
        ...
        if(me.data.includeTotals !== false && Ext.isDefined(me.data.groups) && me.data.summary.length > 0){
    ...
    buildSummaryRows: function (groups, rows, colMerge, level) {
        ...
        for (var i = 0; i < groups.length; i++) {
            ...
            if(me.data.includeTotals !== false){
            ...
            if(me.data.includeTotals !== false && g.summary.length > 0){
                cells = [];
    ...
    buildHeaderRows: function (columns, result) {
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            if (count === 0 || count === 1) {
                s = Ext.String.format('<ss:Cell ss:StyleID="Header"><ss:Data ss:Type="String">{0}</ss:Data><ss:NamedCell ss:Name="Print_Titles" /></ss:Cell>', me.sanitizeHtml(col.text));
            } else {
                s = Ext.String.format('<ss:Cell ss:MergeAcross="{0}" ss:StyleID="Header"><ss:Data ss:Type="String">{1}</ss:Data><ss:NamedCell ss:Name="Print_Titles" /></ss:Cell>', count - 1, me.sanitizeHtml(col.text));
            }
    ...
    buildCell: function (value, merge, style, type) {
        ...
        return Ext.create('Ext.exporter.excel.Cell', {
            value:      value,
            type:       type,
            merge:      merge,
            style:      style
